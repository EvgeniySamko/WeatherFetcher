...
class WeatherScreenViewModel(val interactor: WeatherInteractor) : BaseViewModel<ViewState>() {
    ...
    override fun reduse(event: Event, previousState: ViewState): ViewState? {
        when (event) {
            is UiEvent.OnButtonClicked -> {
                ...
                viewModelScope.launch {
                    interactor.getWeather().fold(
                        onError = {
                            processDateEvent(DataEvent.OnWeatherFetchFailed(error = it))

                        },
                        onSuccess = {
                            processDateEvent(DataEvent.OnWeatherFetchSucceed(temperature = it.temperature))
